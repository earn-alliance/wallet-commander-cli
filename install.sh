...
function install {
	...
	if [ ! $LATEST_VERSION ]; then
		echo "${YELLOW}"
		log "Failed while attempting to install wallet-commander CLI. Please install manually:"
		echo ""
		...
		log "3. chmod +x ./wallet-commander"
		log "4. mv ./wallet-commander /usr/local/bin"
		echo ""
		fail "Exiting..."
	fi
	...
	*) fail "Unknown os: $(uname -s)";;
	...
	case "${OS}_${ARCH}" in
	"linux_amd64")
		URL=$GH/$OWNER/$REPO_NAME/releases/download/$LATEST_VERSION/cli-wallet-commander-linux-amd64.zip
		FTYPE=".zip"
		;;
	"darwin_amd64")
		URL=$GH/$OWNER/$REPO_NAME/releases/download/$LATEST_VERSION/cli-wallet-commander-mac-amd64.zip
		FTYPE=".zip"
		;;
  "darwin_arm64")
		URL=$GH/$OWNER/$REPO_NAME/releases/download/$LATEST_VERSION/cli-wallet-commander-mac-arm64.zip
		FTYPE=".zip"
		;;
